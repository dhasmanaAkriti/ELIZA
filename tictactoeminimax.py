...
class MinimaxPlayer(TicTacToe, Player):
    ...
    def staticEval(self, node):
        ...
        if self.check_win(node.state, self.side):
            return 100
        elif self.check_win(node.state, self.opponent(self.side)):
            return -100
        else:
            return 10
    ...
    def alphaBetaMinimax(self, node, alpha, beta):
        ...
        if successors == [] or successors == None or node.depth == self.limit:
            return node.score
...
if __name__ == '__main__':
    game = TicTacToe(3)
    print(game)
    game.playNGames(5, MinimaxPlayer(3, 2), SimplePlayer(3), False)
    # game.playNGames(10, MinimaxPlayer(8, 4), SimplePlayer(8), False)
